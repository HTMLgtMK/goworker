...
func NewStdinFrontend(engine *engine.Engine, config *config.StdinConfig) *StdinFrontend {
	if config.Theme != "" {
	...
	engine.AddEventListener(func(event model.Event) {
		switch model.EventType(event.Type) {
		case model.EventType(task.EventTaskStatus), model.EventType(task.EventTaskProgress):
...
// Write 实现 model.Context 的 Writer 回调。
...
func (f *StdinFrontend) handleToken(kind model.RenderKind, content string, done bool) {
	switch kind {
	case model.KindText, model.KindThinking:
		if content != "" {
...
// decide 执行一次 HITL 决策会话，实现 model.Context 的 Decide 契约。
...
func (f *StdinFrontend) Run() error {
	...
	for {
		...
		ctx := model.NewContext(context.Background(), f.Write, f.decide, nil)
...
func (f *StdinFrontend) runWithCancel(ctx *model.Context, line string) {
