...
// testHub 构造一个最小可用的 model.Hub，只暴露测试需要的字段。
func testHub(cfg *runtimeconfig.Config) (*model.Hub, *[]*runtimeconfig.Config) {
	...
	hub := &model.Hub{
		...
		Tools: func() []model.Tool { return nil }, // collectTools 依赖，缺了会 nil 函数 panic
	}
	return hub, &saved
}
...
func newAgentPluginP(hub *model.Hub, pv core.Provider) *AgentPlugin {
	cfg, _ := hub.Config.(*runtimeconfig.Config)
	p := &AgentPlugin{hub: hub, cfg: cfg, runGate: make(chan struct{}, 1)}
	p.deps = runtimeagent.SessionDeps{
		Config:       cfg,
...
func newContext(args ...string) (*model.Context, *strings.Builder) {
	...
	return &model.Context{
		...
		FrontendContext: model.FrontendContext{
			...
			WriteToken: func(_ model.RenderKind, s string, _ bool) { buf.WriteString(s) },
		},
	}, &buf
}
...
func buildFakeMCPServer(t *testing.T) string {
	...
	// daemon/internal/core/service → 仓库根（4 级上溯）→ ai-runtime/mcp/testdata/fakeserver
...
func TestInit_OpensMemory(t *testing.T) {
	...
	hub := &model.Hub{
		Config:             cfg,
		RegisterCommand:    func(model.Command) error { return nil },
		SetFallbackHandler: func(func(*model.Context) error) {},
		Tools:              func() []model.Tool { return nil },
	}
